...
//            }
    ...
    public static void translucentWindow(Window window, @ColorInt int colorOn5x) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        if (QMUIDeviceHelper.isMeizu() || QMUIDeviceHelper.isMIUI()) {
     ...
     *
     * @param window 需要被处理的 Activity
     */
    public static boolean setStatusBarLightModeWindow(Window window) {
        ...
        if (mStatuBarType != STATUSBAR_TYPE_DEFAULT) {
            return setStatusBarLightModeWindow(window, mStatuBarType);
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
    ...
    private static boolean setStatusBarLightModeWindow(Window window, @StatusBarType int type) {
        if (type == STATUSBAR_TYPE_MIUI) {
